# 蹭饭地图 Freeload map

一个可以白白到手（免费）的蹭饭地图。

A Freeload map for freeload (free).

# 使用协议 License

我们同意你使用或拷贝这些文件的在写在`LICENSE`文件里的`MIT许可证`之上的前提是**你保证不开价出售它们（这些文件）！**

We agree you using or copy those files with the `MIT LICENSE` in `LICENSE` file table on you premise that **DO NOT sale them (those files) non-free !**

# FreeloadMap

主体内容。

Main content.

# FreeloadMap.Lib

`FreeloadMap.Lib`里主要是`FreeloadMap.Lib.Data`下的`Picture Set 工程`（基于`JSON`）文件的托管类和`图片物品`结构体。

The main things in the `FreeloadMap.Lib` are `Picture Set Project` (based on `JSON`) Files warpper class and `Picture Item` Struct which under `FreeloadMap.Lib.Data`.

# PictureSet

一个简单的制作`Picture Set格式工程`的软件。**也许**之后网站会兼容别的图片集格式。
*是的，这个东西操作起来真的很反人类。这只是个临时工具。*

A simple software to make `Picture Set Format Project`. **Maybe** the website will support any other picture set format later.
*Yes, this garbage software's operation is very anti-human. This is only a temporary tool.*

# 引用 Reference

`Newtonsoft.Json`, License: [MIT LICENSE](https://github.com/JamesNK/Newtonsoft.Json/blob/master/LICENSE.md) (Check on 2021Y.8M.6D 14:45:00), Project url: [https://www.newtonsoft.com/json](https://www.newtonsoft.com/json)